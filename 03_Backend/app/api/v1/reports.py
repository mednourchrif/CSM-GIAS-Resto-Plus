...
async def generate_report(
    ...
    granularity: Literal["daily", "weekly", "monthly"] = Query(
        "daily", description="Grouping: daily, weekly, monthly"
    ),
    user_type: str | None = Query(
        None, description="User type filter (EMPLOYE, STAGIAIRE, VISITEUR)"
    ),
    type_identification: str | None = Query(
        None, description="Identification method filter (QR, FACE)"
    ),
