...
class MealsPerDayChart extends StatelessWidget {
  ...
  const MealsPerDayChart({super.key, required this.items});
...
class UserTypeChart extends StatelessWidget {
  ...
  const UserTypeChart({super.key, required this.items});
...
class RegistrationMethodChart extends StatelessWidget {
  ...
  const RegistrationMethodChart({super.key, required this.items});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: items.asMap().entries.map((e) {
        final ratio = maxCount > 0 ? e.value.count / maxCount : 0.0;
        final label = e.value.method == 'FACE'
...
class PeakHoursChart extends StatelessWidget {
  ...
  const PeakHoursChart({super.key, required this.items});
