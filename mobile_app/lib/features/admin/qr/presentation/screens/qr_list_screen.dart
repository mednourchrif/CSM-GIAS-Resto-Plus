...
class _QrListScreenState extends ConsumerState<QrListScreen> {
  ...
  void _showDetail(QrCode qr) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => _QrDetailScreen(qrCode: qr),
      ),
    );
  }
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: QrCard(qrCode: qr, onTap: () => _showDetail(qr)),
          );
        },
      ),
    );
        ...
        children: [
          DataTable(
            ...
            rows: state.qrCodes.map((qr) {
              return DataRow(
                onSelectChanged: (_) => _showDetail(qr),
                cells: [
...
class _QrDetailScreen extends ConsumerWidget {
  final QrCode qrCode;

  const _QrDetailScreen({required this.qrCode});
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              if (qrCode.qrBase64 != null)
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: Image.memory(
                      base64Decode(
                        qrCode.qrBase64!.contains(',')
                            ? qrCode.qrBase64!.split(',').last
                            : qrCode.qrBase64!,
                      ),
                      width: 240,
                      height: 240,
              ...
              DetailRow(label: 'UUID', value: qrCode.uuid),
              DetailRow(label: 'Type', value: qrCode.typeLabel),
              DetailRow(label: 'Expiration', value: _formatDate(qrCode.dateExpiration)),
              DetailRow(label: 'Validations', value: '${qrCode.nombreValidations}'),
              if (qrCode.createdAt != null)
                DetailRow(label: 'Créé le', value: _formatDateTime(qrCode.createdAt!)),
              if (qrCode.dateRevocation != null)
                DetailRow(label: 'Révoqué le', value: _formatDateTime(qrCode.dateRevocation!)),
              if (qrCode.motifRevocation != null)
                DetailRow(label: 'Motif', value: qrCode.motifRevocation!),
              if (qrCode.derniereValidation != null)
                DetailRow(
                  label: 'Dernière validation',
                  value: _formatDateTime(qrCode.derniereValidation!),
                ),
              const SizedBox(height: Spacing.xxl),
              if (qrCode.isActive) ...[
                Row(
                  children: [
                    Expanded(
                      child: FilledButton.icon(
                        onPressed: () => _handleAction(context, ref, 'download'),
                    ...
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => _handleAction(context, ref, 'print'),
                ...
                Row(
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => _handleAction(context, ref, 'regenerate'),
                    ...
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => _handleAction(context, ref, 'revoke'),
  ...
  Future<void> _download(BuildContext context, WidgetRef ref) async {
    ...
    final safeName = qrCode.proprietaireFullName.replaceAll(RegExp(r'\s+'), '');
    final fileName = 'QR_${safeName}_${qrCode.uuid.substring(0, 8)}.png';
  ...
  void _showPrintPreview(BuildContext context, WidgetRef ref) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => _PrintPreviewScreen(qrCode: qrCode),
      ),
    );
  }
  ...
  void _confirmRegenerate(BuildContext context, WidgetRef ref) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Le QR code actif de ${qrCode.proprietaireFullName} sera révoqué '
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              final qr = await ref.read(qrProvider.notifier).regenerateQr(
                    qrCode.proprietaireUuid,
                    ownerType: qrCode.typeProprietaire,
                  );
              if (context.mounted) {
                if (qr != null) {
                  ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              Navigator.of(ctx).pop();
              final success = await ref.read(qrProvider.notifier).revokeQr(qrCode.uuid);
              if (context.mounted) {
                if (success) {
                  ScaffoldMessenger.of(context).showSnackBar(
...
class _PrintPreviewScreen extends StatelessWidget {
  ...
  const _PrintPreviewScreen({required this.qrCode});
